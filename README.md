...
```bash
NEXT_PUBLIC_SPARK_BYTEBIN_URL=https://your-bytebin.example.com \\
NEXT_PUBLIC_SPARK_BYTESOCKS_URL=wss://your-bytesocks.example.com \\
npm run build
```

或在 Docker 中传递构建参数（参见 `Dockerfile`）：

```bash
docker build --build-arg NEXT_PUBLIC_SPARK_BYTEBIN_URL=https://... -t spark-viewer .
```

`src/env.ts` 中定义了默认值和运行时读取的变量，建议直接修改该文件或通过环境变量覆盖以适配你的部署环境。

如果需要，我可以在 `README.md` 中添加更详细的示例（例如 `.env.local` 示例或 Docker Compose 配置）。
